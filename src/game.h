...
int 	window_init	(x_window_param_t *param, char *title, //делает предварительные работы
					int pos_x, int pos_y, int Width, int Height);//высота, ширина и т.д.
...
int 	str_vec_init	(string_vec_t *vec, unsigned capacity);		//подготовка структуры к работе, выделение изначальной памяти
...
int		str_vec_load_from_file (string_vec_t *vec, char *filedir, char *filename);	//выгружаем слова из файла
...
int load_pixmap(x_window_param_t *window, pixmap_attr_t *pixmap, 	//загружает из файла картинку
...
void 	game_res_free	(x_window_param_t *window, game_res_t *res);			//освобождение ресурсов
...
void 	game_letter_push		(game_stat_t *game, char *letter);			//принимает букву русский яз
void 	game_letter_push_eng	(game_stat_t *game, char letter);				//принимает букву англ яз
int 	game_reset				(game_stat_t *game);				//сбросить текущий прогресс
void 	game_free				(game_stat_t *game);				//освободить ресурсы
char 	*game_return_progress	(game_stat_t *game);						//возвращает текущее слово русский яз
char 	*game_return_progress_eng	(game_stat_t *game);					//возвращает текущее слово англ яз
int		game_win_check			(game_stat_t *game);				//проверяет победу
int		game_lose_check			(game_stat_t *game);				//проверяет проигрыш

int return_letter_by_keycode(unsigned int keycode, char* output);		//принимает код клавиши и возвращает букву
int return_letter_by_keycode_eng(unsigned int keycode, char* output);

void game_draw(x_window_param_t *win, game_res_t *res, game_stat_t *game);	//отрисовка прогресса
